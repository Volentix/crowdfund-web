<template>
  <!-- Sign Up Modal -->
  <b-modal id='signup-modal' :title="$t('sign_up_form')" v-if="!$store.state.apiToken" ref="signUpModal" centered>
    ...
    <div slot="modal-footer" class="w-100">
      <p class="float-left">
        <a @click="switchToLogin()" href="#">{{ $t('login_from_register') }}</a>
      </p>
    </div>
  </b-modal>
</template>
...
export default {
  ...
  mounted () {
    // Watch for `register` event emitted
