<template>
  <!-- Login Modal -->
  <b-modal id="login-modal" :title="$t('login_form')" v-if="!$store.state.apiToken" ref="loginModal" centered>
    <b-form @submit="login($event)" @click="clearLoginError()">
      <b-form-group :label="$t('username_or_email')">
    ...
    <div slot="modal-footer" class="w-100">
      <p class="float-right">
        <a @click="switchToRegister()" href="#">{{ $t('register_from_login') }}</a>
      </p>
